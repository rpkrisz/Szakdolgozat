...
const DropdownUniversities: FC<{
  label: string;
  items: {name: string; id: number; nickName: string}[] | undefined;
}> = ({label, items}) => {
  return (
    <div className="dropdown dropdown-bottom">
      ...
      <ul
        tabIndex={0}
        className="menu dropdown-content text-base-content bg-base-300 rounded-box z-[1] mt-4 w-52 p-2 shadow"
      >
        {items?.map(({name, id}) => (
          <li key={id}>
